...
def get_expression(chromosomes, effect_size):
    ...
    # If not, move on
    try:
        doses={k:sum(v.values()) for k,v in chromosomes.items()}
        expressions=zip(doses.keys(), np.array(doses.values())*effect_size)
    except AttributeError:
        expressions=zip(chromosomes.keys(), np.array(chromosomes.values())*effect_size)
...
#        np.savetxt(f_p, phenotype_dat.reshape(1, phenotype_dat.shape[0]), fmt='%.2f '*len(phens)+'%g')
...
'''
def mutate_SNP(chroms, mut_params, mut_id, f_genos):
    mu, = mut_params
    ...
    mut_ids=range(mut_id, mut_id+num_mutations)
    global snp_id
    snp_id += num_mutations
...
def main(N, optimal_expression, fitness_variance, mutational_parameters, mutational_effect, generations, w, path, sel):
    ...
    while generation < generations:
        ...
        orig_gen_var = np.var( ( get_expression(chromosomes = pop, effect_size = mutational_effect)[:,1] / mutational_effect ) ) 
        ...
        try:
            mutate_SNP(pop, mutational_parameters, snp_id, f_g)
        except TypeError:
            break
        except ValueError:
            pass
        
        mut_gen_var= np.var( ( get_expression(chromosomes = pop, effect_size = mutational_effect)[:,1] / mutational_effect ) )
        ...
        if generation > 90000:
            gen_vars = np.array( [ generation, orig_gen_var, mut_gen_var, drift_gen_var ] )
            np.savetxt( f_c, gen_vars.reshape((1, gen_vars.shape[0])), fmt='%g' )
            
            np.savetxt( f_g, new_gens.reshape(1, new_gens.shape[0]), fmt='%.2f' )
...
import cProfile
cProfile.runctx('main(N, optimal_expression, fitness_variance, mutational_parameters, mutational_effect, generations, w, path, sel)', globals(), {'N':args.N, 'optimal_expression':args.o, 'fitness_variance':args.f, 'mutational_parameters':args.m, 'mutational_effect':args.b, 'generations':args.g, 'w': args.w, 'path':args.p, 'sel':args.s}, 'profilestats')
